...
export default function TopicQueueBoard() {
  ...
  const filteredTopics = useMemo(() => {
    return topics.filter(topic => {
      const siteMatch = selectedSite === 'all' || topic.siteSlug === selectedSite;
      const typeMatch = pageTypeFilter === 'all' || topic.pageType === pageTypeFilter;
  ...
  const handleDrop = (e: React.DragEvent, targetStatus: ColumnStatus) => {
    ...
    setTopics(prev => prev.map(topic => 
      topic.id === draggedTopic.id 
        ? { ...topic, status: targetStatus, updatedAt: new Date().toISOString() }
        : topic
    ));
  ...
  const getStatusBadgeVariant = (status: ColumnStatus): "default" | "secondary" | "destructive" | "outline" => {
    switch (status) {
                  ...
                  >
                    ...
                    <CardContent className="space-y-2">
                      <div className="flex flex-wrap gap-1">
                        ...
                        <Badge variant={getStatusBadgeVariant(topic.status)} className="text-xs">
                          {topic.status}
                        </Badge>
                      </div>
                      <div className="text-xs text-muted-foreground">
                        <div className="font-medium">{topic.primaryKeyword}</div>
                      ...
                      <div className="text-xs text-muted-foreground">
                        {SITES.find(s => s.value === topic.siteSlug)?.label || topic.siteSlug}
                      </div>
                    </CardContent>
                  </Card>
                ))}
