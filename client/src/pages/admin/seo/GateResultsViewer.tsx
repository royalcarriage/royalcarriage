...
const MOCK_GATE_RESULT: GateResult = {
  ...
  checks: {
    ...
    keywordMatch: {
      passed: true,
      details:
        "Primary keyword appears 8 times, secondary keywords well distributed",
    },
  },
  suggestions: [
...
export default function GateResultsViewer({
  draftId,
  gateResults = MOCK_GATE_RESULT,
}: GateResultsViewerProps) {
            ...
            {checkDetails.map((check) => {
              ...
              return (
                <Card key={check.key}>
                  <Collapsible
                    open={isExpanded}
                    onOpenChange={() => toggleCheck(check.key)}
                  >
                    ...
                    {hasDetails && (
                      <CollapsibleContent>
                        <CardContent className="pt-0">
                          ...
                          {"score" in check.data &&
                            check.data.score !== undefined && (
                              <div className="text-sm space-y-1">
                                <div>Score: {check.data.score.toFixed(2)}</div>
                                <div>
                                  Threshold: {check.data.threshold.toFixed(2)}
                                </div>
                          ...
                          {"errors" in check.data &&
                            check.data.errors.length > 0 && (
