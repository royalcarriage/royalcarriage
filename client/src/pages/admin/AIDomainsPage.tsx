...
export default function AIDomainsPage() {
  ...
  async function handleImportHosting() {
    const projectId = prompt('Enter GCP project id to import hosting domains from');
    if (!projectId) return;
    setLoading(true);
    try {
      const res = await fetch('/api/ai/domains/import-hosting', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const data = await res.json();
      if (data?.domains) setDomains(data.domains);
      alert(`Imported ${data?.imported || 0} domains`);
    } catch (err) {
          ...
          <Card className="mb-6">
            ...
            <CardContent>
              ...
              <div className="flex gap-2">
                ...
                <Button variant="secondary" onClick={handleImportHosting}>Import from Firebase Hosting</Button>
              </div>
            </CardContent>
          </Card>
