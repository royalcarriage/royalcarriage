...
export default function UptimeMonitor() {
  ...
  const checkSite = async (url: string, name: string): Promise<SiteStatus> => {
    try {
      ...
      const failuresLast24h = Math.floor(
        ((100 - uptimePercentage) * checksLast24h) / 100,
      );
  ...
  const overallUptime =
    sites.length > 0
      ? sites.reduce((sum, site) => sum + site.uptimePercentage, 0) /
        sites.length
      : 0;
    ...
    <div className="space-y-6">
      ...
      {sitesDown > 0 && (
        <Alert variant="destructive">
          ...
          <AlertDescription>
            <strong>
              ⚠️ {sitesDown} site{sitesDown > 1 ? "s" : ""} currently down!
            </strong>
            <br />
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card>
          ...
          <CardContent>
            <div
              className={`text-3xl font-bold ${getUptimeColor(overallUptime)}`}
            >
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-3xl font-bold">
              {sites.length > 0
                ? Math.round(
                    sites.reduce((sum, s) => sum + s.responseTime, 0) /
                      sites.length,
                  )
                : 0}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Real-time status of all monitored URLs
          </CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
