...
type WizardStep = 'select-type' | 'upload' | 'detect' | 'preview' | 'validate' | 'submit';
type DatasetType = 'campaigns' | 'keywords' | 'search_terms' | 'devices' | 'locations';
...
export default function AdsImportWizard() {
  ...
  const steps: { key: WizardStep; label: string; description: string }[] = [
    { key: 'select-type', label: 'Select Type', description: 'Choose dataset type' },
    { key: 'upload', label: 'Upload CSV', description: 'Select your ads data file' },
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Alert>
              ...
              <AlertDescription>
                Required columns: {getCurrentConfig()?.requiredColumns.join(', ')}
              </AlertDescription>
            </Alert>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {parsedData.headers.map((header, index) => {
                const isRequired = getCurrentConfig()?.requiredColumns.includes(header);
                return (
                  <div
                    key={index}
                  ...
                  >
                    ...
                    <span className={`font-medium ${isRequired ? 'text-green-700' : ''}`}>
                      {header}
                    </span>
                  </div>
                );
