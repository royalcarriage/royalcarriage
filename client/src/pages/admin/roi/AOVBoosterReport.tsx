...
const generateMockOpportunities = (): UpgradeOpportunity[] => {
  const mockBookings: Booking[] = Array.from({ length: 50 }, (_, i) => ({
    ...
    vehicleType: ["sedan", "suv", "luxury sedan"][
  ...
  return mockBookings
    .map((booking) => {
      ...
      let opportunityType: UpgradeOpportunity["opportunityType"] =
        "service_upgrade";
...
export function AOVBoosterReport() {
  const [opportunities] = useState<UpgradeOpportunity[]>(
      ...
      <Card>
        ...
        <CardContent>
            ...
            <Card>
              <CardContent className="pt-6">
                <div className="text-sm text-gray-600">
                  Avg Increase per Opportunity
                </div>
                <div className="text-3xl font-bold">
                  $
                  {(
                    totalPotentialIncrease / filteredOpportunities.length || 0
                  ).toFixed(0)}
                </div>
              </CardContent>
            </Card>
              ...
              <tbody>
                {filteredOpportunities.map((opp, idx) => (
                  <tr key={idx} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">
                      <Badge
                        className={OPPORTUNITY_TYPE_COLORS[opp.opportunityType]}
                      >
          ...
          <div className="mt-4 text-sm text-gray-600">
            Showing {filteredOpportunities.length} opportunities with total
            potential increase of ${totalPotentialIncrease.toLocaleString()}
          </div>
        </CardContent>
      </Card>
