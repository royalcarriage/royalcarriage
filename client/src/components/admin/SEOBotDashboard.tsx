...
import { FileText, CheckCircle, AlertTriangle, Play, Eye, Upload } from 'lucide-react';
...
export function SEOBotDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'propose' | 'drafts' | 'quality' | 'publish'>('drafts');
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-600 mt-1">Manage content proposals, drafts, and publishing workflow</p>
      </div>
      ...
      <div className="flex gap-2 border-b border-gray-200">
        ...
        >
          Manage Drafts ({topics.filter(t => t.status === 'draft' || t.status === 'proposed').length})
        </button>
        <button
        ...
        >
          Quality Gates ({topics.filter(t => t.status === 'quality_check' || t.status === 'ready').length})
        </button>
        <button
      ...
      {activeTab === 'propose' && (
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Generate New Topic Proposals</h3>
            <p className="text-gray-600 mb-4">
              Analyzes keyword clusters, Moovs service mix, and profit model to propose high-value content topics.
            </p>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
            <h4 className="font-semibold text-blue-900 mb-2">💡 Proposal Criteria</h4>
            <ul className="text-sm text-blue-800 space-y-1">
              <li>• Profit score ≥70 (based on keyword ROAS + service margin)</li>
      ...
      {activeTab === 'drafts' && (
        <div className="space-y-4">
          {topics.filter(t => t.status === 'draft' || t.status === 'proposed').map(topic => (
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  ...
                  <div className="flex gap-4 text-sm text-gray-600">
                    <span>Intent: <strong>{topic.intent}</strong></span>
                    <span>Profit Score: <strong className="text-green-600">{topic.profitScore}/100</strong></span>
                    <span>Est. Traffic: <strong>{topic.estimatedTraffic}/mo</strong></span>
                  </div>
                </div>
              </div>
          ...
          {topics.filter(t => t.status === 'draft' || t.status === 'proposed').length === 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
              <FileText className="w-12 h-12 text-gray-400 mx-auto mb-3" />
              <p className="text-gray-600">No drafts available. Run the topic proposer to generate new content ideas.</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'quality' && (
        <div className="space-y-4">
          <div className="bg-white rounded-lg border border-gray-200 p-6">
            <h3 className="text-lg font-semibold mb-4">Run Quality Gate Checks</h3>
            <p className="text-gray-600 mb-4">
              Validates content against Google spam policies, checks for duplicates, enforces word count minimums, and verifies schema/links.
            </p>
            <button className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors flex items-center gap-2">
          ...
          {topics.filter(t => t.status === 'quality_check' || t.status === 'ready').map(topic => (
            <div key={topic.id} className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
              ...
              {topic.status === 'quality_check' && (
                <div className="bg-yellow-50 border border-yellow-200 rounded p-3 mb-4">
                  <div className="flex items-center gap-2 text-yellow-800">
                    ...
                    <span className="font-medium">Quality check in progress...</span>
                  </div>
                </div>
              )}
      ...
      {activeTab === 'publish' && (
        <div className="space-y-4">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-4">
            <h4 className="font-semibold text-yellow-900 mb-2">⚠️ Publishing Safety</h4>
          ...
          {topics.filter(t => t.status === 'ready').map(topic => (
            <div key={topic.id} className="bg-white rounded-lg border border-gray-200 p-6">
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                  ...
                  <div className="flex gap-4 text-sm text-gray-600">
                    <span>Profit Score: <strong className="text-green-600">{topic.profitScore}/100</strong></span>
                    <span>Target Site: <strong>Airport</strong></span>
                  </div>
                </div>
              </div>
          ...
          {topics.filter(t => t.status === 'ready').length === 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-8 text-center">
              ...
              <p className="text-gray-600">No content ready for publishing. Complete quality gates first.</p>
            </div>
          )}
        </div>
      )}
