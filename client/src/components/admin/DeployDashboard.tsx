...
import { Rocket, GitBranch, Clock, CheckCircle, XCircle, AlertCircle, RefreshCw } from 'lucide-react';
...
const DeployDashboard: React.FC = () => {
  ...
  const sites = [
    { id: 'airport', name: 'Airport Service', domain: 'chicagoairportblackcar.com', status: 'live' },
    { id: 'partybus', name: 'Party Bus', domain: 'partybus.com', status: 'pending' },
    { id: 'executive', name: 'Executive Service', domain: 'executive.com', status: 'pending' },
    { id: 'wedding', name: 'Wedding Transport', domain: 'wedding.com', status: 'pending' }
  ];
  ...
  const handleDeploy = (siteId: string) => {
    ...
    setTimeout(() => {
      ...
      alert(`Deployment started for ${sites.find(s => s.id === siteId)?.name}`);
    }, 2000);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Deployment Control</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {sites.map(site => (
          <div key={site.id} className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-start justify-between mb-4">
              ...
              <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                site.status === 'live' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'
              }`}>
                {site.status}
              </span>
            </div>
            ...
            <div className="space-y-2 mb-4 text-sm text-gray-600">
              <div className="flex items-center justify-between">
                <span>Branch:</span>
                <span className="font-mono text-xs bg-gray-100 px-2 py-1 rounded">main</span>
              </div>
              <div className="flex items-center justify-between">
            ...
            >
              ...
              {isDeploying ? 'Deploying...' : site.status === 'live' ? 'Deploy Now' : 'Not Ready'}
            </button>
          </div>
        ))}
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg">
        ...
        <div className="divide-y divide-gray-200">
          {deployHistory
            .filter(deploy => selectedSite === 'all' || deploy.site === sites.find(s => s.id === selectedSite)?.name)
            .map(deploy => (
              <div key={deploy.id} className="px-6 py-4 hover:bg-gray-50">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="font-medium text-gray-900">{deploy.site}</div>
                  ...
                  <div className="text-right">
                    <span className={`inline-flex items-center gap-1 px-3 py-1 text-sm font-medium rounded-full ${getStatusColor(deploy.status)}`}>
                      {deploy.status}
                    </span>
                  </div>
                </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        ...
        <div className="space-y-4">
          <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div>
              <h4 className="font-medium text-blue-900 mb-1">Manual Approval Required</h4>
              <p className="text-sm text-blue-800">
                All deployments require manual approval for safety. Automatic deployments are disabled
                to prevent accidental spam content publishing.
              </p>
            </div>
          </div>
