...
export function AdminLayout({ children }: AdminLayoutProps) {
  ...
  const navigation = [
    { name: 'Dashboard', href: '/admin', icon: LayoutDashboard, roles: ['user', 'admin', 'super_admin'] },
    { name: 'Page Analyzer', href: '/admin/analyze', icon: Brain, roles: ['admin', 'super_admin'] },
    { name: 'User Management', href: '/admin/users', icon: Users, roles: ['admin', 'super_admin'] },
    ...
    <div className="flex h-screen bg-gray-50">
      ...
      <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-slate-900 text-white transition-all duration-300 flex flex-col`}>
        ...
        <div className="p-4 border-b border-slate-800 flex items-center justify-between">
          {sidebarOpen && (
            <div>
              <h2 className="text-xl font-bold text-amber-500">Royal Carriage</h2>
