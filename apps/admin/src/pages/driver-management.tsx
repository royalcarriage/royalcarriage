...
function DriverModal({
  ...
  async function handleImageUpload(): Promise<string | null> {
    ...
    try {
      const driverId =
        `${formData.personalInfo?.firstName?.toLowerCase()}-${formData.personalInfo?.lastName?.toLowerCase()}` ||
        `driver-${Date.now()}`;
      const storageRef = ref(
        storage,
        `driver-images/${driverId}-${Date.now()}.${imageFile.name.split(".").pop()}`,
      );
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...
                      employment: {
                        ...formData.employment!,
                        status: e.target
                          .value as Driver["employment"]["status"],
                      },
                    })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    setFormData({
                      ...
                      employment: {
                        ...
                        employmentType: e.target
                          .value as Driver["employment"]["employmentType"],
                      },
                    })
                  }
                  className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              {SPECIALIZATIONS.map((spec) => (
                <button
                  ...
                  onClick={() =>
                    setFormData({
                      ...formData,
                      specializations: toggleArrayItem(
...
export default function DriverManagementPage() {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        ...
        items.sort((a, b) => {
          ...
          const statusCompare =
            statusOrder.indexOf(a.employment?.status || "inactive") -
            statusOrder.indexOf(b.employment?.status || "inactive");
          if (statusCompare !== 0) return statusCompare;
          return `${a.personalInfo?.lastName} ${a.personalInfo?.firstName}`.localeCompare(
  ...
  async function handleDeleteDriver(driver: Driver) {
    ...
    if (
      !confirm(
        `Are you sure you want to delete "${driver.personalInfo?.firstName} ${driver.personalInfo?.lastName}"? This cannot be undone.`,
      )
    ) {
      return;
    }
  ...
  async function handleStatusChange(
    driver: Driver,
    newStatus: Driver["employment"]["status"],
  ) {
    if (!canEdit) {
        ...
        <div className="grid grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="text-3xl font-bold text-blue-600">
              {
                drivers.filter(
                  (d) => d.availability?.currentStatus === "online",
                ).length
              }
            </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="text-3xl font-bold text-purple-600">
              {drivers.length > 0
                ? (
                    drivers.reduce(
                      (sum, d) => sum + (d.ratings?.averageRating || 0),
                      0,
                    ) / drivers.length
                  ).toFixed(2)
                : "0.00"}
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="text-3xl font-bold text-orange-600">
              {drivers
                .reduce((sum, d) => sum + (d.performance?.totalRides || 0), 0)
                .toLocaleString()}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredDrivers.map((driver) => {
              const statusColor = getStatusColor(
                driver.employment?.status || "inactive",
              );
              const availColor = getAvailabilityColor(
                driver.availability?.currentStatus || "offline",
              );
                  ...
                  <div className="p-6">
                    <div className="flex items-start gap-4 mb-4">
                      <div className="w-16 h-16 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                        {driver.profileImage ? (
                          <img
                            ...
                            alt={`${driver.personalInfo?.firstName}`}
                            className="w-full h-full object-cover"
                          />
                        ) : (
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex gap-2 mt-1">
                          ...
                          <span
                            className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-${availColor}-100 text-${availColor}-800`}
                          >
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                      ...
                      <div className="bg-gray-50 p-3 rounded">
                        ...
                        <div className="font-bold text-gray-900">
                          {driver.performance?.completedRides?.toLocaleString() ||
                            0}
                        </div>
                      </div>
                    </div>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-500">
                              {driver.employment?.employmentType}
                            </div>
                          </div>
                        </div>
                      </td>
