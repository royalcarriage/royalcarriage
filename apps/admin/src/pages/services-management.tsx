            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    website: e.target.value as ServiceItem["website"],
                  })
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    locationCount: parseInt(e.target.value) || 0,
                  })
                }
                className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              />
...
export default function ServicesManagementPage() {
  ...
  const filteredServices = services.filter((service) => {
    const matchesWebsite =
      selectedWebsite === "all" || service.website === selectedWebsite;
    const matchesSearch =
      searchQuery === "" ||
  ...
  async function handleGenerateContent() {
    ...
    try {
      ...
      const response = await fetch(
        "/.netlify/functions/generate-service-content",
        {
          method: "POST",
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600">
              Manage {services.length} services across 4 websites and generate
              AI content
            </p>
          </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-orange-50 p-6 rounded-lg">
            <div className="text-3xl font-bold text-orange-600">
              {services.reduce((sum, s) => sum + s.contentGenerated, 0)} /{" "}
              {services.reduce((sum, s) => sum + s.contentTotal, 0)}
            </div>
        ...
        <div className="flex gap-4 mb-6">
          ...
          >
            {generatingContent
              ? "Generating..."
              : `Generate Content (${selectedServices.size})`}
          </button>
        </div>
                ...
                {filteredServices.map((service) => {
                  ...
                  return (
                    <tr key={service.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-${color}-100 text-${color}-800 capitalize`}
                        >
                          {service.website}
                        </span>
                      </td>
