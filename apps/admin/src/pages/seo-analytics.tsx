    ...
    try {
      ...
      for (const websiteId of websites) {
        ...
        const uniqueLocations = new Set(
          snapshot.docs.map((doc) => doc.data().locationId),
        ).size;
      ...
      for (const locDoc of locationsSnapshot.docs) {
        ...
        const totalServiceCombos =
          (location.applicableServices.airport || 0) +
        ...
        locProgress.push({
          ...
          progress:
            totalServiceCombos > 0
              ? Math.round((approved / totalServiceCombos) * 100)
              : 0,
        });
      }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Track content generation progress and SEO coverage across all 240+
            locations
          </p>
        </div>
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-blue-50 p-6 rounded-lg border border-blue-200">
            <div className="text-2xl font-bold text-blue-600">
              {metrics.estimatedTotalPages.toLocaleString()}
            </div>
            <div className="text-sm text-gray-600">Total Possible Pages</div>
            <div className="text-xs text-gray-500 mt-1">
              {metrics.totalLocations} locations × {metrics.totalServices}{" "}
              services
            </div>
          </div>
          <div className="bg-green-50 p-6 rounded-lg border border-green-200">
            <div className="text-2xl font-bold text-green-600">
              {metrics.contentApproved.toLocaleString()}
            </div>
            <div className="text-sm text-gray-600">Approved Content</div>
            <div className="text-xs text-gray-500 mt-1">
              {metrics.coveragePercentage}% coverage
            </div>
          </div>
          <div className="bg-yellow-50 p-6 rounded-lg border border-yellow-200">
            <div className="text-2xl font-bold text-yellow-600">
              {metrics.contentPending}
            </div>
            <div className="text-sm text-gray-600">Pending Approval</div>
            <div className="text-xs text-gray-500 mt-1">
              {metrics.contentGenerated} total generated
            </div>
          </div>
          <div className="bg-purple-50 p-6 rounded-lg border border-purple-200">
            ...
            <div className="text-xs text-gray-500 mt-1">
              {metrics.pagesGenerated} generated
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border rounded-lg p-6 mb-8">
          ...
          <div className="w-full bg-gray-200 rounded-full h-8 overflow-hidden">
            ...
            >
              {metrics.coveragePercentage > 10 &&
                `${metrics.coveragePercentage}%`}
            </div>
          </div>
          <div className="mt-2 text-sm text-gray-600">
            {metrics.contentApproved} of {metrics.estimatedTotalPages} pages
                ...
                <div className="flex items-start justify-between mb-3">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Coverage: {site.locationsCovered} locations
                    </p>
                  </div>
