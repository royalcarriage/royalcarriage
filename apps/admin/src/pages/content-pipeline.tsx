...
const WORKFLOW_STEPS = [
  {
    ...
    description: "Choose which websites to generate content for",
  },
  {
    id: 2,
    title: "Select Locations",
    description: "Pick specific locations or select all",
  },
  {
    id: 3,
    title: "Select Services",
    description: "Choose services or generate for all applicable",
  },
  {
    id: 4,
    title: "Preview & Confirm",
    description: "Review your selections before generation",
  },
  {
    id: 5,
...
export default function ContentPipelinePage() {
  ...
  async function handleGenerate() {
    ...
    const locationsArray = selectAllLocations
      ? locations.map((l) => l.id)
      : Array.from(selectedLocations);
    const servicesArray = selectAllServices
      ? filteredServices.map((s) => s.id)
      : Array.from(selectedServices);

    const totalItems =
      websitesArray.length * locationsArray.length * servicesArray.length;
    setGenerationProgress({ current: 0, total: totalItems });

    try {
      const response = await fetch(
        "/.netlify/functions/generate-pipeline-content",
        {
          method: "POST",
      ...
      alert(
        `Successfully queued ${result.totalItems || totalItems} content items for generation`,
      );
      setGenerating(false);
      setCurrentStep(1);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            AI-powered workflow to generate SEO content at scale across all
            websites
          </p>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-md p-8 mb-8">
                    ...
                    <div className="flex items-center justify-between">
                      <div className="text-left">
                        ...
                        <div className="text-sm text-gray-600">
                          {
                            services.filter((s) => s.website === website.id)
                              .length
                          }{" "}
                          services
                        </div>
          ...
          {currentStep === 2 && (
            <div>
              ...
              <p className="text-gray-600 mb-6">
                Choose specific locations or generate for all {locations.length}{" "}
                locations
              </p>
                ...
                <label className="flex items-center gap-3 p-4 bg-blue-50 border-2 border-blue-200 rounded-lg cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-blue-700">
                      Generate content for all {locations.length} Chicago
                      locations
                    </div>
                  </div>
                </label>
              ...
              <div className="mt-4 text-sm text-gray-600">
                {selectAllLocations ? (
                  ...
                  <span>
                    {selectedLocations.size} of {locations.length} locations
                    selected
                  </span>
                )}
              </div>
            ...
            <div>
                ...
                <label className="flex items-center gap-3 p-4 bg-purple-50 border-2 border-purple-200 rounded-lg cursor-pointer">
                  ...
                  <div>
                    ...
                    <div className="text-sm text-purple-700">
                      Generate content for all {filteredServices.length}{" "}
                      services in selected websites
                    </div>
                  </div>
                </label>
              </div>
              ...
              <div className="mt-4 text-sm text-gray-600">
                {selectAllServices ? (
                  ...
                  <span>
                    {selectedServices.size} of {filteredServices.length}{" "}
                    services selected
                  </span>
                )}
              </div>
