...
export default function PerformanceMonitoringPage() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'traffic' | 'keywords' | 'content'>('overview');
  ...
  const [contentAgeAnalysis, setContentAgeAnalysis] = useState<ContentAgeAnalysis | null>(null);
    ...
    try {
      ...
      const metrics = metricsSnapshot.docs.map((doc) => {
        ...
        return {
          ...
          url: data.url || '',
          title: data.title || '',
          search: data.search || { impressions: 0, clicks: 0, avgPosition: 0, ctr: 0 },
          engagement: data.engagement || { pageViews: 0, sessions: 0, avgSessionDuration: 0, bounceRate: 0, conversionRate: 0 },
      ...
      setContentAgeAnalysis({
        ...
        aging: ageData.filter((c) => (c.ageInDays || 0) > 30 && (c.ageInDays || 0) <= 90).length,
        stale: ageData.filter((c) => (c.ageInDays || 0) > 90).length,
      });

      // Load traffic trends
      const trendsSnapshot = await getDocs(collection(db, 'performance_trends'));
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Track SEO performance, keyword rankings, and content engagement metrics
          </p>
        </div>
          ...
          <>
              ...
              <>
                ...
                <div className="grid grid-cols-4 gap-4 mb-8">
                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <div className="text-sm text-gray-500 uppercase tracking-wide">Impressions</div>
                    <div className="text-3xl font-bold text-gray-900 mt-1">
                      {formatNumber(summary.totalImpressions)}
                    </div>
                    <div className="text-sm text-green-600 mt-1">Search visibility</div>
                  </div>

                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <div className="text-sm text-gray-500 uppercase tracking-wide">Clicks</div>
                    <div className="text-3xl font-bold text-gray-900 mt-1">
                      {formatNumber(summary.totalClicks)}
                    </div>
                    <div className="text-sm text-blue-600 mt-1">From search results</div>
                  </div>

                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <div className="text-sm text-gray-500 uppercase tracking-wide">Avg CTR</div>
                    <div className="text-3xl font-bold text-gray-900 mt-1">
                      {summary.avgCtr.toFixed(2)}%
                    </div>
                    <div className={`text-sm mt-1 ${summary.avgCtr >= 2 ? 'text-green-600' : 'text-yellow-600'}`}>
                      {summary.avgCtr >= 2 ? 'Good' : 'Needs improvement'}
                    </div>
                  </div>

                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <div className="text-sm text-gray-500 uppercase tracking-wide">Avg Position</div>
                    <div className="text-3xl font-bold text-gray-900 mt-1">
                      {summary.avgPosition.toFixed(1)}
                    </div>
                    <div className={`text-sm mt-1 ${summary.avgPosition <= 20 ? 'text-green-600' : 'text-yellow-600'}`}>
                      {summary.avgPosition <= 10 ? 'Top 10' : summary.avgPosition <= 20 ? 'Top 20' : 'Needs work'}
                    </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-4 mb-8">
                  <div className="bg-gradient-to-br from-purple-50 to-purple-100 rounded-xl border border-purple-200 p-6">
                    <div className="text-sm text-purple-700 uppercase tracking-wide">Page Views</div>
                  ...
                  <div className="bg-gradient-to-br from-orange-50 to-orange-100 rounded-xl border border-orange-200 p-6">
                    <div className="text-sm text-orange-700 uppercase tracking-wide">Avg Bounce Rate</div>
                  ...
                  <div className="bg-gradient-to-br from-teal-50 to-teal-100 rounded-xl border border-teal-200 p-6">
                    <div className="text-sm text-teal-700 uppercase tracking-wide">Avg Session</div>
                    <div className="text-3xl font-bold text-teal-900 mt-1">
                      {Math.floor(summary.avgSessionDuration / 60)}:{String(Math.floor(summary.avgSessionDuration % 60)).padStart(2, '0')}
                    </div>
                  </div>

                  <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 rounded-xl border border-indigo-200 p-6">
                    <div className="text-sm text-indigo-700 uppercase tracking-wide">Total Pages</div>
                    <div className="text-3xl font-bold text-indigo-900 mt-1">{summary.totalPages}</div>
                  </div>
                </div>
                  ...
                  <div className="bg-white rounded-xl shadow-sm border p-6 mb-8">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                      <div className="text-center">
                        <div className="text-3xl font-bold text-green-600">{rankingDistribution.top10}</div>
                        <div className="text-sm text-gray-600">Position 1-10</div>
                      ...
                      <div className="text-center">
                        <div className="text-3xl font-bold text-blue-600">{rankingDistribution.top20}</div>
                        <div className="text-sm text-gray-600">Position 11-20</div>
                        <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                          <div
                      ...
                      <div className="text-center">
                        <div className="text-3xl font-bold text-yellow-600">{rankingDistribution.top50}</div>
                        <div className="text-sm text-gray-600">Position 21-50</div>
                        <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                          <div
                      ...
                      <div className="text-center">
                        <div className="text-3xl font-bold text-red-600">{rankingDistribution.beyond50}</div>
                        <div className="text-sm text-gray-600">Position 50+</div>
                        <div className="w-full bg-gray-200 rounded-full h-2 mt-2">
                          <div
                ...
                {contentAgeAnalysis && (
                  <div className="bg-white rounded-xl shadow-sm border p-6 mb-8">
                    <h3 className="text-lg font-bold text-gray-900 mb-4">Content Freshness</h3>
                    <div className="grid grid-cols-3 gap-4">
                      <div className="bg-green-50 rounded-lg p-4 text-center">
                        <div className="text-3xl font-bold text-green-600">{contentAgeAnalysis.fresh}</div>
                        <div className="text-sm text-gray-600">Fresh (0-30 days)</div>
                      </div>
                      <div className="bg-yellow-50 rounded-lg p-4 text-center">
                        <div className="text-3xl font-bold text-yellow-600">{contentAgeAnalysis.aging}</div>
                        <div className="text-sm text-gray-600">Aging (30-90 days)</div>
                      </div>
                      <div className="bg-red-50 rounded-lg p-4 text-center">
                        <div className="text-3xl font-bold text-red-600">{contentAgeAnalysis.stale}</div>
                        <div className="text-sm text-gray-600">Stale (90+ days)</div>
                      </div>
                    </div>
                  </div>
                )}
            ...
            {activeTab === 'traffic' && (
              <>
                ...
                <div className="bg-white rounded-xl shadow-sm border p-6 mb-8">
                  ...
                  <div className="h-64 flex items-end gap-1">
                    {trafficData.slice(-30).map((point, index) => {
                      const maxImpressions = Math.max(...trafficData.map((d) => d.impressions));
                      const height = maxImpressions > 0 ? (point.impressions / maxImpressions) * 100 : 0;
                      return (
                        <div
                          key={index}
                ...
                <div className="grid grid-cols-3 gap-6 mb-8">
                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <h4 className="text-sm font-medium text-gray-500 mb-4">IMPRESSIONS TREND</h4>
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(trafficData.reduce((sum, d) => sum + d.impressions, 0))}
                    </div>
                    <div className="text-sm text-green-600 mt-2">Total in period</div>
                  </div>

                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <h4 className="text-sm font-medium text-gray-500 mb-4">CLICKS TREND</h4>
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(trafficData.reduce((sum, d) => sum + d.clicks, 0))}
                    </div>
                    <div className="text-sm text-blue-600 mt-2">Total in period</div>
                  </div>

                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <h4 className="text-sm font-medium text-gray-500 mb-4">PAGE VIEWS TREND</h4>
                    <div className="text-3xl font-bold text-gray-900">
                      {formatNumber(trafficData.reduce((sum, d) => sum + d.pageViews, 0))}
                    </div>
                    <div className="text-sm text-purple-600 mt-2">Total in period</div>
                  </div>
                </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="divide-y divide-gray-200">
                        {trafficData.slice(-14).reverse().map((row, idx) => (
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="px-6 py-4 text-sm text-gray-900">{row.date}</td>
                            <td className="px-6 py-4 text-sm text-gray-900 text-right">{formatNumber(row.impressions)}</td>
                            <td className="px-6 py-4 text-sm text-gray-900 text-right">{formatNumber(row.clicks)}</td>
                            <td className="px-6 py-4 text-sm text-gray-900 text-right">
                              {row.impressions > 0 ? ((row.clicks / row.impressions) * 100).toFixed(2) : '0.00'}%
                            </td>
                            <td className="px-6 py-4 text-sm text-gray-900 text-right">{formatNumber(row.pageViews)}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
            ...
            {activeTab === 'keywords' && (
              <>
                ...
                <div className="grid grid-cols-4 gap-4 mb-8">
                  <div className="bg-white rounded-xl shadow-sm border p-6">
                    <div className="text-sm text-gray-500 uppercase">Total Keywords</div>
                    <div className="text-3xl font-bold text-gray-900 mt-1">{keywordRankings.length}</div>
                  </div>
                  <div className="bg-green-50 rounded-xl border border-green-200 p-6">
                    <div className="text-sm text-green-700 uppercase">Top 10</div>
                    <div className="text-3xl font-bold text-green-900 mt-1">
                      {keywordRankings.filter((k) => k.position <= 10).length}
                    </div>
                  </div>
                  <div className="bg-blue-50 rounded-xl border border-blue-200 p-6">
                    <div className="text-sm text-blue-700 uppercase">Moving Up</div>
                    <div className="text-3xl font-bold text-blue-900 mt-1">
                      {keywordRankings.filter((k) => k.change > 0).length}
                    </div>
                  </div>
                  <div className="bg-red-50 rounded-xl border border-red-200 p-6">
                    <div className="text-sm text-red-700 uppercase">Declining</div>
                ...
                <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                        ...
                        {keywordRankings.map((keyword, idx) => (
                          <tr key={idx} className="hover:bg-gray-50">
                            <td className="px-6 py-4">
                              ...
                              <div className="text-xs text-gray-500 truncate max-w-xs">{keyword.url}</div>
                            </td>
                  ...
                  {keywordRankings.length === 0 && (
                    <div className="text-center py-12 text-gray-500">
                      No keyword rankings data available yet. Run a sync to populate data.
                    </div>
                  )}
                </div>
              </>
            ...
            {activeTab === 'content' && (
              <>
                ...
                <div className="grid grid-cols-2 gap-6">
                  ...
                  <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                    <div className="px-6 py-4 border-b bg-green-50">
                      <h3 className="text-lg font-bold text-green-900">Top Performing Content</h3>
                      <p className="text-sm text-green-700">Highest clicks and engagement</p>
                    </div>
                        ...
                        <div key={idx} className="p-4 hover:bg-gray-50">
                          <div className="flex justify-between items-start mb-2">
                            <div className="flex-1">
                              <h4 className="font-medium text-gray-900 text-sm">{item.title || item.locationId}</h4>
                              <p className="text-xs text-gray-500">{item.url}</p>
                            </div>
                            <span className="text-lg font-bold text-green-600">{item.search?.clicks || 0}</span>
                          </div>
                          <div className="flex gap-4 text-xs text-gray-600">
                            <span>Impressions: {formatNumber(item.search?.impressions || 0)}</span>
                            <span>CTR: {(item.search?.ctr || 0).toFixed(2)}%</span>
                            <span>Position: {(item.search?.avgPosition || 0).toFixed(1)}</span>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="bg-white rounded-xl shadow-sm border overflow-hidden">
                    <div className="px-6 py-4 border-b bg-yellow-50">
                      <h3 className="text-lg font-bold text-yellow-900">Needs Improvement</h3>
                      <p className="text-sm text-yellow-700">Low CTR or high bounce rate</p>
                    </div>
                    <div className="divide-y divide-gray-100">
                        ...
                        <div key={idx} className="p-4 hover:bg-gray-50">
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`text-sm px-2 py-1 rounded ${
                              (item.search?.ctr || 0) < 1 ? 'bg-yellow-100 text-yellow-800' : 'bg-red-100 text-red-800'
                            }`}>
                              {(item.search?.ctr || 0) < 1 ? 'Low CTR' : 'High Bounce'}
                            </span>
                          </div>
                          <div className="flex gap-4 text-xs text-gray-600">
                            <span>Impressions: {formatNumber(item.search?.impressions || 0)}</span>
                            <span>CTR: {(item.search?.ctr || 0).toFixed(2)}%</span>
                            <span>Bounce: {(item.engagement?.bounceRate || 0).toFixed(1)}%</span>
                          </div>
                        </div>
                      ))}
                      {needsImprovement.length === 0 && (
                        <div className="p-8 text-center text-gray-500">All content performing well!</div>
                      )}
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-8 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl border border-indigo-200 p-6">
                  <h3 className="text-lg font-bold text-indigo-900 mb-4">Performance Recommendations</h3>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-white rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-2">Improve CTR</h4>
                    ...
                    <div className="bg-white rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-2">Reduce Bounce Rate</h4>
                      <ul className="text-sm text-gray-600 space-y-1">
                    ...
                    <div className="bg-white rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-2">Boost Rankings</h4>
                      <ul className="text-sm text-gray-600 space-y-1">
                    ...
                    <div className="bg-white rounded-lg p-4">
                      <h4 className="font-medium text-gray-900 mb-2">Increase Engagement</h4>
                      <ul className="text-sm text-gray-600 space-y-1">
