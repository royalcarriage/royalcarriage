...
export default function AdvancedAnalyticsPage() {
  ...
  const [contentAnalytics, setContentAnalytics] =
    useState<ContentAnalyticsData | null>(null);
  ...
  const [customReport, setCustomReport] = useState<CustomReportResult | null>(
    null,
  );
  ...
  async function loadContentAnalytics() {
    ...
    try {
      ...
      const getContentAnalyticsFn = httpsCallable(
        functions,
        "getContentAnalytics",
      );
  ...
  async function loadContentAnalyticsFromFirestore() {
    try {
      ...
      const categoryMetrics: ContentPerformanceMetrics[] = Object.entries(
        byCategory,
      ).map(([category, items]) => ({
        category,
        totalPages: items.length,
        avgQualityScore:
          items.reduce((sum, i) => sum + (i.overallScore || 0), 0) /
          items.length,
        avgEngagement: 0,
        totalViews: items.reduce((sum, i) => sum + (i.views || 0), 0),
        totalConversions: items.reduce(
      ...
      setContentAnalytics({
        summary: {
          ...
          totalConversions: scores.reduce(
            (sum, s) => sum + (s.conversions || 0),
            0,
          ),
  ...
  async function loadBenchmark() {
    ...
    try {
      const functions = getFunctions();
      const getCompetitorBenchmarkFn = httpsCallable(
        functions,
  ...
  async function loadTrends() {
    ...
    try {
      const functions = getFunctions();
      const getAnalyticsTrendsFn = httpsCallable(
        functions,
        "getAnalyticsTrends",
      );
  ...
  async function generateCustomReport() {
    ...
    try {
      const functions = getFunctions();
      const generateCustomReportFn = httpsCallable(
        functions,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            Comprehensive analytics, ROI analysis, competitor benchmarking, and
            trend forecasting
          </p>
        </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center p-3 bg-green-50 rounded-lg">
                    ...
                    <span className="font-bold text-lg text-green-600">
                      {(
                        (roiAnalysis.netProfit / roiAnalysis.totalRevenue) *
                        100
                      ).toFixed(1)}
                      %
                    </span>
                  ...
                  >
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        <span className="text-gray-600">Pages:</span>
                        <span className="font-semibold">
                          {comp.metrics.pageCount}
                        </span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-gray-600">Quality:</span>
                        <span className="font-semibold">
                          {comp.metrics.contentQuality}
                        </span>
                      </div>
              ...
              <div className="border rounded-lg p-6">
                <div className="flex justify-between items-center mb-4">
                  ...
                  <span className="text-sm text-gray-500">
                    Generated:{" "}
                    {new Date(customReport.generatedAt).toLocaleString()}
                  </span>
                </div>
                      ...
                      <tr className="border-b bg-gray-50">
                        {customReport.data.length > 0 &&
                          Object.keys(customReport.data[0]).map((key) => (
                            <th
                              key={key}
                              className="text-left py-3 px-4 font-semibold capitalize"
                            >
                              {key.replace(/([A-Z])/g, " $1")}
                            </th>
                          ))}
                ...
                {customReport.data.length > 20 && (
                  <p className="text-sm text-gray-500 mt-4">
                    Showing 20 of {customReport.data.length} rows. Export for
                    full data.
                  </p>
                )}
              </div>
            ...
            {trendData && (
              <>
                ...
                <div className="grid grid-cols-4 gap-4 mb-8">
                  <div className="bg-gradient-to-br from-blue-50 to-blue-100 p-6 rounded-lg border border-blue-200">
                    <div
                      className={`text-4xl font-bold ${getTrendColor(trendData.trend)}`}
                    >
                  ...
                  <div className="border rounded-lg p-6">
                    ...
                    <div className="space-y-1">
                      {trendData.forecast.slice(0, 15).map((item, idx) => {
                        const maxValue = Math.max(
                          ...trendData.forecast.map((d) => d.upperBound),
                        );
                        const width =
                          maxValue > 0 ? (item.predicted / maxValue) * 100 : 0;
                        const lowerWidth =
                          maxValue > 0 ? (item.lowerBound / maxValue) * 100 : 0;
                        const upperWidth =
                          maxValue > 0 ? (item.upperBound / maxValue) * 100 : 0;
                        return (
                          <div key={idx} className="flex items-center gap-2">
                            <span className="w-20 text-xs text-gray-500">
                              {item.date.slice(5)}
                  ...
                  <div className="mt-6 border rounded-lg p-6 bg-yellow-50 border-yellow-200">
                    ...
                    {trendData.seasonality.peakPeriods && (
                      <p>
                        Peak Periods:{" "}
                        <strong>
                          {trendData.seasonality.peakPeriods.join(", ")}
                        </strong>
                      </p>
                    )}
                  </div>
