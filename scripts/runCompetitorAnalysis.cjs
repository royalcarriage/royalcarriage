...
const COMPETITORS = [
  {
    ...
    keywordsRanking: ['chicago airport limo', 'ohare car service', 'wedding limo chicago'],
  ...
  },
  {
    ...
    keywordsRanking: ['party bus chicago', 'chicago airport shuttle', 'corporate car service'],
  ...
  },
  {
    ...
    keywordsRanking: ['luxury car service chicago', 'limo service chicago', 'chicago chauffeur'],
...
const KEYWORD_OPPORTUNITIES = [
  { keyword: 'ohare airport limo service', searchVolume: 2400, difficulty: 0.65, ourRank: null, competitorRank: 3 },
  { keyword: 'midway airport car service', searchVolume: 1800, difficulty: 0.55, ourRank: null, competitorRank: 5 },
  { keyword: 'chicago executive transportation', searchVolume: 1200, difficulty: 0.70, ourRank: null, competitorRank: 4 },
  { keyword: 'downtown chicago limo', searchVolume: 900, difficulty: 0.60, ourRank: null, competitorRank: 2 },
  { keyword: 'naperville airport shuttle', searchVolume: 600, difficulty: 0.40, ourRank: null, competitorRank: null },
  { keyword: 'schaumburg corporate car service', searchVolume: 450, difficulty: 0.35, ourRank: null, competitorRank: null },
  { keyword: 'gold coast limo service', searchVolume: 350, difficulty: 0.45, ourRank: null, competitorRank: null },
  { keyword: 'lincoln park party bus', searchVolume: 500, difficulty: 0.50, ourRank: null, competitorRank: 8 },
  { keyword: 'chicago wedding guest transportation', searchVolume: 700, difficulty: 0.55, ourRank: null, competitorRank: 6 },
  { keyword: 'bachelor party bus chicago suburbs', searchVolume: 400, difficulty: 0.45, ourRank: null, competitorRank: null },
];
...
async function analyzeCompetitors() {
  ...
  for (const competitor of COMPETITORS) {
    ...
    // Store in Firestore
    await db.collection('competitor_analysis').doc(competitor.website.replace('.', '-')).set({
      ...analysis,
    ...
    console.log(`  Strengths: ${analysis.strengths.join(', ') || 'None identified'}`);
    console.log(`  Weaknesses: ${analysis.weaknesses.join(', ') || 'None identified'}`);
    console.log();
  }
...
async function identifyServiceGaps() {
  ...
  const coveredCombinations = new Set(contentSnapshot.docs.map(doc => doc.id));
  ...
  for (const gap of topGaps) {
    console.log(`  [${gap.priority.toUpperCase()}] ${gap.locationName} + ${gap.serviceName} (relevance: ${gap.relevance})`);
  }

  // Store gaps summary
  await db.collection('competitor_analysis').doc('service_gaps_summary').set({
...
async function identifyKeywordOpportunities() {
  ...
  for (const kw of KEYWORD_OPPORTUNITIES) {
    ...
    const opportunityScore = Math.round(
      (kw.searchVolume / 100) * (1 - kw.difficulty) * (kw.competitorRank ? (10 - kw.competitorRank) / 10 : 1.5)
    );

    opportunities.push({
      ...kw,
      opportunityScore,
      recommendation: opportunityScore > 15 ? 'High Priority' : opportunityScore > 8 ? 'Medium Priority' : 'Low Priority',
    });
  }
  ...
  for (const opp of opportunities) {
    ...
    console.log(`    Volume: ${opp.searchVolume} | Difficulty: ${(opp.difficulty * 100).toFixed(0)}% | Score: ${opp.opportunityScore}`);
  }

  // Store in Firestore
  await db.collection('competitor_analysis').doc('keyword_opportunities').set({
    opportunities,
    totalOpportunities: opportunities.length,
    highPriority: opportunities.filter(o => o.recommendation === 'High Priority').length,
    analyzedAt: admin.firestore.Timestamp.now(),
  });
