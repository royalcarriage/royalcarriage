...
async function verifyFleetVehicles() {
  ...
  try {
    ...
    Object.entries(vehicles).forEach(([id, vehicle]) => {
      console.log(`  ✓ ${vehicle.name} (${vehicle.category}) - ${vehicle.capacity} passengers`);
    });
...
async function verifyServices() {
  ...
  try {
    ...
    console.log('\nBy Website:');
    console.log(`  Airport (chicagoairportblackcar): ${websites['chicagoairportblackcar']}`);
    console.log(`  Corporate (chicagoexecutivecarservice): ${websites['chicagoexecutivecarservice']}`);
    console.log(`  Wedding (chicagoweddingtransportation): ${websites['chicagoweddingtransportation']}`);
    console.log(`  Party Bus (chicago-partybus): ${websites['chicago-partybus']}`);
    if (websites['other'] > 0) {
...
async function main() {
  ...
  try {
    ...
    if (fleetResults.success) {
      console.log(`  ✓ ${fleetResults.total}/${fleetResults.expected} vehicles verified`);
    } else {
      console.log(`  ✗ ${fleetResults.total}/${fleetResults.expected} vehicles found`);
    }
    ...
    if (servicesResults.success) {
      ...
      console.log(`  ✗ ${servicesResults.total}/${servicesResults.expected} services found`);
    }
    ...
    if (fleetResults.success && servicesResults.success) {
      console.log('╔════════════════════════════════════════════════════════════╗');
      console.log('║         ALL DATA SUCCESSFULLY VERIFIED!                    ║');
      console.log('╚════════════════════════════════════════════════════════════╝\n');
      process.exit(0);
    } else {
      console.log('╔════════════════════════════════════════════════════════════╗');
      console.log('║         VERIFICATION INCOMPLETE OR FAILED                  ║');
      console.log('╚════════════════════════════════════════════════════════════╝\n');
      process.exit(1);
    }
  } catch (error) {
