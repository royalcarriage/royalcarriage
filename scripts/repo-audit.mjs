...
if (codex) {
  ...
  if (m) {
    ...
    if (val.startsWith('{') || val.startsWith('[')) {
      issues.push('.codex/config.toml: `model` appears to be a map/array; expected a string.');
    }
  }
} else {
...
if (rules) {
  if (/allow\s+read\s*:\s*if\s+true/.test(rules) || /allow\s+write\s*:\s*if\s+true/.test(rules)) {
    issues.push('firestore.rules: contains global allow read/write -> enforce least-privilege.');
  }
} else {
...
const wfPaths = fs.readdirSync(path.join(root, '.github/workflows')).map(f=>`.github/workflows/${f}`);
for (const wf of wfPaths) {
  const txt = read(wf);
  if (txt && /FIREBASE_SERVICE_ACCOUNT|FIREBASE_PROJECT_ID|GCP_SA_KEY|WORKLOAD_IDENTITY_PROVIDER/.test(txt)) {
...
console.log('\nRecommendations:');
console.log('- Inspect local user configs at ~/.codex/config.toml for `model` typed as map and convert to string.');
console.log('- Run `npm run gates` and fix lint/typecheck if you replace placeholders with real tools.');
