...
function parseCSV(filePath) {
  try {
    ...
    console.error(
      `❌ Error parsing CSV ${path.basename(filePath)}:`,
      error.message,
    );
    return { headers: [], rows: [] };
  }
  ...
  try {
    ...
    return {
      service_margins: {
        airport_transfer: {
          margin: 0.28,
          driver_payout_pct: 0.52,
          tax_pct: 0.1,
        },
        corporate_hourly: { margin: 0.3, driver_payout_pct: 0.5, tax_pct: 0.1 },
        wedding_event: { margin: 0.33, driver_payout_pct: 0.47, tax_pct: 0.1 },
        party_bus: { margin: 0.35, driver_payout_pct: 0.45, tax_pct: 0.1 },
      },
      default_assumptions: {
...
function importGoogleAds() {
  ...
  const files = fs
    .readdirSync(DATA_DIRS.googleAds)
    .filter(
      (f) => f.endsWith(".csv") && !f.startsWith(".") && f !== "README.md",
    );
  ...
  for (const file of files) {
    ...
    const isKeywordReport = headers.some(
      (h) =>
        h.toLowerCase().includes("search term") ||
        h.toLowerCase().includes("keyword"),
    );

    const isCampaignReport = headers.some(
      (h) =>
    ...
    if (isKeywordReport) {
      ...
      for (const row of rows) {
        ...
        const impressions = parseNumber(
          extractValue(row, headers, ["impressions", "impr."]),
        );
        const conversions = parseNumber(
          extractValue(row, headers, ["conversions", "conv."]),
        );
        const convValue = parseNumber(
          extractValue(row, headers, [
            "conv. value",
            "conversion value",
            "total conv. value",
          ]),
      ...
      for (const row of rows) {
        const campaign = extractValue(row, headers, ["campaign"]);
        const status = extractValue(row, headers, [
          "status",
          "campaign status",
        ]);
        ...
        const convValue = parseNumber(
          extractValue(row, headers, ["conv. value", "conversion value"]),
        );
      ...
      console.log(
        `   ✓ Imported ${allCampaigns.length} campaigns from ${file}`,
      );
    } else {
      console.log(
        `   ⚠️  Could not detect report type for ${file} (unsupported format)`,
      );
    }
  }
...
function importMoovs() {
  ...
  for (const file of files) {
    ...
    for (const row of rows) {
      ...
      const date = extractValue(row, headers, [
        "date",
        "pickup date",
        "service date",
      ]);
      const status = extractValue(row, headers, ["status"]);
      const serviceType = extractValue(row, headers, [
        "service type",
        "service",
        "type",
      ]);
      const vehicleType = extractValue(row, headers, [
        "vehicle type",
        "vehicle",
      ]);
      ...
      const driverPayout = parseNumber(
        extractValue(row, headers, ["driver payout", "driver pay"]),
      );
      const tax = parseNumber(
        extractValue(row, headers, ["tax", "tax amount"]),
      );
      const reqSource = extractValue(row, headers, [
        "req source",
        "source",
        "request source",
        "utm_source",
      ]);
      const origin = extractValue(row, headers, [
        "origin",
        "pickup",
        "pickup location",
      ]);
      const destination = extractValue(row, headers, [
        "destination",
        "dropoff",
        "dropoff location",
      ]);
      ...
      if (status && status.toUpperCase() === "DONE" && revenue > 0) {
        const isGoogleAds =
          reqSource.toLowerCase().includes("google") ||
  ...
  console.log(
    `   ✓ Google Ads attributed revenue: $${googleAdsRevenue.toFixed(2)}`,
  );
...
function importKeywordResearch() {
  ...
  const files = fs
    .readdirSync(DATA_DIRS.keywordResearch)
    .filter(
      (f) =>
  ...
  if (files.length === 0) {
    console.log(
      "⚠️  No keyword research files found in data/keyword-research/",
    );
    console.log(
      "   See data/keyword-research/README.md for export instructions",
    );
    return {
      keywords: [],
      dataQuality: "missing",
  ...
  for (const file of files) {
    ...
    for (const row of rows) {
      ...
      const competition = extractValue(row, headers, [
        "competition",
        "keyword difficulty",
      ]);
      const cpc = parseNumber(
        extractValue(row, headers, ["cpc", "suggested bid", "top of page bid"]),
      );
...
function inferIntent(keyword) {
  ...
  if (
    kw.includes("corporate") ||
    kw.includes("executive") ||
...
`;
  ...
  fs.writeFileSync(
    path.join(OUTPUT_DIRS.content, "ads_landing_page_matrix.csv"),
    csv,
  );
}
...
function generateROISummary(googleAdsData, moovsData, profitModel) {
  ...
  const summary = {
    ...
    roas:
      moovsData.googleAdsRevenue > 0
        ? moovsData.googleAdsRevenue / googleAdsData.totalSpend
        : 0,
  };
