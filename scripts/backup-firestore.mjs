...
if (!admin.apps.length) {
  try {
    admin.initializeApp({
      ...
      storageBucket: process.env.FIREBASE_STORAGE_BUCKET || 'royalcarriagelimoseo.appspot.com'
    });
  } catch (error) {
...
try {
  ...
  console.warn('Storage not initialized (skipping cloud upload):', error.message);
}
...
async function uploadToStorage(localBackupDir, timestamp) {
  try {
    ...
    // Mock implementation
    const files = fs.readdirSync(localBackupDir);
    console.log(`     ℹ️  Would upload ${files.length} files to gs://bucket/backups/${timestamp}/`);
...
async function main() {
  ...
  try {
    ...
    console.log(`     ✅ Total collections: ${manifest.totals.collectionCount}`);
    ...
    if (uploadResult.success) {
      ...
      console.log(`   Storage path: gs://${uploadResult.bucket}/${uploadResult.path}`);
