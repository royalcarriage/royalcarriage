...
if (!admin.apps.length) {
  try {
    ...
    console.warn('Firebase Admin initialization skipped (emulator mode or credentials not configured)');
  }
}
...
async function fetchTopPages() {
  try {
    ...
    // Mock data for skeleton implementation
    return [
      { page: '/ohare-airport-limo', clicks: 450, impressions: 8200, position: 3.2 },
      { page: '/midway-airport-limo', clicks: 380, impressions: 6500, position: 4.1 },
      { page: '/corporate-car-service', clicks: 320, impressions: 5800, position: 3.8 },
      { page: '/wedding-limo-service', clicks: 280, impressions: 4900, position: 4.5 },
...
async function main() {
  ...
  try {
    ...
    console.log(`📅 Current week: ${weekRanges.current.start.toLocaleDateString()} - ${weekRanges.current.end.toLocaleDateString()}`);
    console.log(`📅 Previous week: ${weekRanges.previous.start.toLocaleDateString()} - ${weekRanges.previous.end.toLocaleDateString()}\n`);
    
    // Fetch data
    console.log('📊 Fetching booking data...');
    const currentBookings = await fetchBookingData(weekRanges.current.start, weekRanges.current.end);
    const previousBookings = await fetchBookingData(weekRanges.previous.start, weekRanges.previous.end);
    
    console.log('💰 Fetching ads data...');
    const currentAds = await fetchAdsData(weekRanges.current.start, weekRanges.current.end);
    const previousAds = await fetchAdsData(weekRanges.previous.start, weekRanges.previous.end);
    ...
    console.log(`\n📈 Summary:`);
    console.log(`   Revenue: ${formatCurrency(currentData.revenue)} (${calculateChange(currentData.revenue, previousData.revenue).percentage.toFixed(1)}%)`);
    console.log(`   Spend: ${formatCurrency(currentData.spend)}`);
    console.log(`   Profit: ${formatCurrency(currentData.revenue - currentData.spend)}`);
