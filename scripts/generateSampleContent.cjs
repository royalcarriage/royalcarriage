...
const TOP_LOCATIONS = {
  airport: ['chicago-loop', 'ohare', 'midway', 'naperville', 'schaumburg', 'oak-brook', 'evanston', 'oak-park', 'rosemont', 'downtown'],
  corporate: ['chicago-loop', 'river-north', 'magnificent-mile', 'west-loop', 'streeterville', 'gold-coast', 'lincoln-park', 'wicker-park', 'fulton-market', 'south-loop'],
  wedding: ['chicago-loop', 'gold-coast', 'lincoln-park', 'naperville', 'oak-brook', 'lake-forest', 'hinsdale', 'barrington', 'geneva', 'st-charles'],
  partyBus: ['chicago-loop', 'river-north', 'wicker-park', 'lincoln-park', 'wrigleyville', 'old-town', 'bucktown', 'logan-square', 'pilsen', 'south-loop']
};

// Top services for each website
const TOP_SERVICES = {
  airport: ['airport-ohare', 'airport-midway', 'airport-downtown-hotel', 'airport-business-meeting', 'airport-corporate-group'],
  corporate: ['corporate-executive-airport', 'corporate-meeting', 'corporate-client', 'corporate-hourly', 'corporate-conference'],
  wedding: ['wedding-bride', 'wedding-bridal-party', 'wedding-guest-transport', 'wedding-reception-entrance', 'wedding-honeymoon-airport'],
  partyBus: ['partybus-bachelor', 'partybus-bachelorette', 'partybus-birthday', 'partybus-brewery', 'partybus-nightclub']
};
...
function generateInternalLinks(serviceId, locationId, allServices, allLocations) {
  const links = [];
  ...
  const relatedLocations = allLocations.filter(l => l.id !== locationId).slice(0, 3);
...
async function main() {
  ...
  for (const [websiteType, locationIds] of Object.entries(TOP_LOCATIONS)) {
    ...
    for (const serviceId of serviceIds) {
      ...
      for (const locationId of locationIds.slice(0, 5)) { // Top 5 locations per service
        ...
        try {
          ...
          const contentDoc = {
            ...
            breadcrumbs: [
              ...
              { name: location.name, url: `/service/${locationId}/${serviceId}` }
            ]
          };
          ...
          errors++;
          console.error(`  ✗ Error: ${serviceId}/${locationId} - ${err.message}`);
        }
      }
    }
  }
  ...
  for (const [type, wsId] of Object.entries(WEBSITE_IDS)) {
    const wsCount = await db.collection('service_content')
