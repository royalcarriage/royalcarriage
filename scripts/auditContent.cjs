...
async function audit() {
  ...
  locSnap.forEach((doc) => {
    if (count < 5) {
      const d = doc.data();
      console.log(
        `  - ${d.name} (${d.slug || doc.id}): type=${d.type}, area=${d.area}`,
      );
      count++;
    }
  });
  ...
  svcSnap.forEach((doc) => {
    if (count < 10) {
      const d = doc.data();
      console.log(
        `  - ${d.name} (${d.slug || doc.id}): website=${d.websiteId}, category=${d.category}`,
      );
      count++;
    }
  });
  ...
  fleetSnap.forEach((doc) => {
    const d = doc.data();
    console.log(
      `  - ${d.name}: ${d.type || d.category}, capacity=${d.passengerCapacity || d.capacity}`,
    );
  });
