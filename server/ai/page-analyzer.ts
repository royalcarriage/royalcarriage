...
export class PageAnalyzer {
  ...
  private calculateSEOScore(content: string, metrics: any): number {
    ...
    if (
      metrics.headingStructure.h2.length >= 2 &&
      metrics.headingStructure.h2.length <= 6
    ) {
      score += 15;
    ...
    const hasMetaDescription = /<meta\s+name=["']description["']/i.test(
      content,
    );
  ...
  private generateRecommendations(
    ...
    if (totalKeywords < 3) {
      recommendations.seo.push(
        "Increase keyword usage naturally throughout content",
      );
    ...
    if (metrics.wordCount < 300) {
      recommendations.content.push(
        "Expand content to at least 300-500 words for better SEO",
      );
    ...
    if (!hasLocation) {
      recommendations.content.push(
        "Add location-specific content (Chicago, O'Hare, Midway)",
      );
    }
    ...
    if (!hasVehicle) {
      recommendations.content.push(
        "Include vehicle types in content (sedan, SUV, limousine)",
      );
    }
    ...
    if (!hasPhoneNumber) {
      recommendations.conversion.push(
        "Add prominent phone number for immediate bookings",
      );
    }
    ...
    if (!hasBookingCTA) {
      recommendations.conversion.push(
        "Add clear 'Book Now' or 'Get Quote' call-to-action",
      );
    }

    recommendations.conversion.push(
      "Include trust badges (licensed, insured, certified)",
    );
    recommendations.conversion.push("Add customer testimonials or reviews");
    recommendations.conversion.push(
      "Display pricing transparency or flat-rate messaging",
    );
  ...
  generateVehicleContent(vehicle: string): string {
    const descriptions = {
      sedan:
        "Our luxury sedans offer comfortable transportation for up to 3 passengers with ample luggage space. Perfect for airport transfers and business meetings.",
      ...
      limousine:
        "Experience ultimate luxury in our stretch limousines. Perfect for special occasions, corporate events, and making a lasting impression.",
    };

    return (
      descriptions[vehicle.toLowerCase() as keyof typeof descriptions] ||
