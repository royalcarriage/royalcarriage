...
export const summarizeCustomerReviews = functions.https.onCall(async (data, context) => {
  try {
    ...
    if (reviewDocs.empty) {
      return { summary: 'No reviews found', sentiment: 'neutral', keyPoints: [] };
    }

    const reviews = reviewDocs.docs.map(doc => doc.data().text).join('\n\n');
...
async function getCachedOrGenerate(cacheKey: string, generator: () => Promise<any>) {
  try {
  ...
  return [
    {
      ...
      answer: 'You can book through our website or by calling our customer service team.',
    },
    // ... more templates
  ];
}
