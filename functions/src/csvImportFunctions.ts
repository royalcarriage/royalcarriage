      ...
      for (let j = 0; j < chunk.length; j++) {
        ...
        try {
          ...
          batch.set(db.collection("bookings").doc(tripId), {
            ...
            status:
              row["Status Slug"] === "done"
                ? "completed"
                : row["Status Slug"] || "pending",
  ...
  async (data, context): Promise<CSVParseResult> => {
    ...
    for (let i = 0; i < csvData.length; i++) {
      ...
      try {
        ...
        batch.set(
          db.collection("ads_metrics").doc(docId),
          {
            campaignId,
...
export const getImportHistory = functions.https.onCall(
  async (data, context) => {
    if (!context.auth)
...
export const getImportErrorReport = functions.https.onCall(
  async (data, context) => {
    if (!context.auth)
      throw new functions.https.HttpsError("unauthenticated", "Login required");
    const { importId } = data;
