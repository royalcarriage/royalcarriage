...
export const createPartnerProfile = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "POST") {
    ...
    try {
      ...
      if (!name || !email || !commissionRate || !commissionType) {
        return res.status(400).json({
          error:
            "Missing required fields: name, email, commissionRate, commissionType",
        });
      }
...
export const getPartnerProfiles = functions.https.onRequest(
  async (req, res) => {
    if (req.method !== "GET") {
      return res.status(405).send("Method Not Allowed");
    }
...
export const createSettlementReceivable = functions.https.onRequest(
  (req, res) => {
    res.status(501).send("Not Implemented: Create Settlement Receivable");
  },
);

export const getSettlementsReceivable = functions.https.onRequest(
  (req, res) => {
