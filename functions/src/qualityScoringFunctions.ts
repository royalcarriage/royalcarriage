  ...
  async (data: { contentId: string; contentData?: any }, context) => {
    ...
    try {
      ...
      const scores: ContentQualityScores = {
        keywordDensity: calculateKeywordDensity(content.content || '', content.keywords || []),
...
export const bulkScoreContent = functions.https.onCall(
  async (data: { websiteId?: string; locationId?: string; maxItems?: number }, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError('unauthenticated', 'Must be logged in');
    }
    ...
    try {
      let query;
      if (websiteId) {
        query = db.collection('service_content').where('websiteId', '==', websiteId);
      } else if (locationId) {
        query = db.collection('service_content').where('locationId', '==', locationId);
      } else {
...
export const getQualityScoreSummary = functions.https.onCall(
  async (data: { websiteId?: string }, context) => {
    ...
    try {
      ...
      let qry;
      if (data.websiteId) {
        qry = db.collection('content_quality_scores').where('websiteId', '==', data.websiteId);
      } else {
      ...
      const lowestScoring = scores.sort((a, b) => a.overallScore - b.overallScore).slice(0, 10).map((s) => ({
      ...
      const highestScoring = scores.sort((a, b) => b.overallScore - a.overallScore).slice(0, 10).map((s) => ({
        contentId: s.contentId,
        score: s.overallScore,
        location: s.locationId,
        service: s.serviceId,
...
function calculateReadability(content: string): number {
  ...
  const index = 0.4 * (words / sentences + 100 * (avgWordLength - 4.71) / words);
...
function calculateStructure(content: string): number {
  ...
  const score = Math.min(headingCount * 5 + listCount * 3 + paragraphCount / 3, 100);
...
function calculateSeoOptimization(content: any): number {
  ...
  if (content.title && content.title.length >= 50 && content.title.length <= 60) {
    score += 25;
  } else if (content.title && content.title.length >= 40 && content.title.length <= 70) {
    score += 15;
  }

  // Meta description (155-160 chars) - 25 points
  if (content.metaDescription && content.metaDescription.length >= 155 && content.metaDescription.length <= 160) {
    score += 25;
  } else if (content.metaDescription && content.metaDescription.length >= 140 && content.metaDescription.length <= 170) {
    score += 15;
  }

  // Keywords (15-20) - 20 points
  if (content.keywords && content.keywords.length >= 15 && content.keywords.length <= 20) {
    score += 20;
  } else if (content.keywords && content.keywords.length >= 10 && content.keywords.length <= 25) {
    score += 12;
  }

  // Internal links (8-12) - 15 points
  if (content.internalLinks && content.internalLinks.length >= 8 && content.internalLinks.length <= 12) {
    score += 15;
  } else if (content.internalLinks && content.internalLinks.length >= 5 && content.internalLinks.length <= 15) {
    score += 8;
  }
...
function calculateEngagement(content: any): number {
  ...
  if (content.content && content.content.toLowerCase().includes('book') && content.content.toLowerCase().includes('now')) {
    score += 40;
  } else {
    score += 20;
  }
...
function generateRecommendations(scores: ContentQualityScores, content: any): string[] {
  const recommendations: string[] = [];

  // Keyword density
  if (scores.keywordDensity < 70) {
    recommendations.push('Increase keyword density - consider adding 2-3 more keyword mentions');
  }
  ...
  if (scores.contentLength < 70) {
    recommendations.push('Expand content - add 300-500 more words to reach optimal length');
  }

  // Structure
  if (scores.structure < 70) {
    recommendations.push('Improve structure - add more headings, lists, and subheadings');
  }

  // SEO optimization
  if (scores.seoOptimization < 70) {
    recommendations.push('Optimize for SEO - review title, description, and schema markup');
  }

  // Originality
  if (scores.originality < 70) {
    recommendations.push('Improve uniqueness - rewrite sections with more original phrasing');
  }

  // Engagement
  if (scores.engagement < 70) {
    recommendations.push('Enhance engagement - add more CTAs, questions, and formatted lists');
  }
