...
export const decommissionVehicle = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
...
export const scheduleMaintenance = functions.https.onCall(async (data, context) => {
    ...
    await db.collection("maintenance").add({
        vehicleId, type, scheduledDate: date, notes, status: "scheduled"
    });
    return { success: true };
});

export const completeMaintenance = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
...
export const getVehicleMaintenanceHistory = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
...
export const logFuelEntry = functions.https.onCall(async (data, context) => {
    ...
    await db.collection("fuel_logs").add({
        vehicleId, gallons, cost, mileage, driverId,
        timestamp: admin.firestore.FieldValue.serverTimestamp()
    });
    // Update vehicle current mileage
    await db.collection("vehicles").doc(vehicleId).update({ currentMileage: mileage });
    return { success: true };
});
...
export const updateRegistration = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
...
export const triggerServiceAlerts = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
    const vehicles = await db.collection("vehicles").where("status", "==", "active").get();
