  ...
  async (
    ...
    try {
      ...
      for (const competitor of competitors) {
        const competitorAnalysis = await analyzeCompetitorWebsite(competitor, analysisDepth);
      ...
      const analysis = {
        ...
        opportunities: {
          ...
          highPriorityCount: opportunities.filter((o) => o.opportunity > 1000).length,
          mediumPriorityCount: opportunities.filter((o) => o.opportunity > 500 && o.opportunity <= 1000).length,
          lowPriorityCount: opportunities.filter((o) => o.opportunity <= 500).length,
        },
        gaps: {
        ...
        comparison: {
          ...
          weRankForButTheyDont: opportunities
            ...
            .map((o) => o.keyword)
            .slice(0, 30),
          commonKeywords: opportunities.filter((o) => o.theyRankFor && o.weRankFor).length,
        },
      ...
      await db.collection('competitor_analysis').doc(analysis.analysisId).set({
        ...analysis,
        savedAt: new Date(),
      });
  ...
  async (data: { analysisId?: string; limit?: number }, context) => {
    ...
    try {
      ...
      if (data.analysisId) {
        ...
        const analysisDoc = await db.collection('competitor_analysis').doc(data.analysisId).get();
        ...
        const analyses = snapshot.docs
          .map((doc) => doc.data())
          .sort((a, b) => (b.analysisDate?.toMillis?.() || 0) - (a.analysisDate?.toMillis?.() || 0))
    ...
    try {
      ...
      if (!analysisId) {
        ...
        const latest = snapshot.docs.sort(
          (a, b) => (b.data().analysisDate?.toMillis?.() || 0) - (a.data().analysisDate?.toMillis?.() || 0)
        )[0];
        analysisId = latest.id;
      }

      const analysisDoc = await db.collection('competitor_analysis').doc(analysisId).get();
      if (!analysisDoc.exists) {
        throw new Error('Analysis not found');
      }
    ...
    topPages: [
      {
        ...
        topKeywords: ['chicago limo service', 'luxury car service chicago', 'black car service'],
      },
      // ... more pages
    ],
...
function calculateServiceGaps(
  ...
  return gaps.filter((gap, index, self) => self.findIndex((g) => g.serviceType === gap.serviceType) === index);
}
...
function generateRecommendations(
  ...
  const quickWins = opportunities.filter(
    (o) => o.searchVolume > 500 && o.difficulty < 40 && o.theyRankFor && !o.weRankFor
  );
  if (quickWins.length > 0) {
    recommendations.push(`Create content for ${quickWins.length} quick-win keywords (high volume, low difficulty)`);
  }
  ...
  if (allServices.size > 5) {
    recommendations.push(`Expand service offerings - competitors offer ${allServices.size} services`);
  }
  ...
  if (competitors.some((c) => c.metrics?.contentQuality === 'Excellent')) {
    recommendations.push('Improve content quality - competitors have excellent-quality content');
  }

  // Backlink strategy
  const avgBacklinks = competitors.reduce((sum, c) => sum + (c.metrics?.backlinks || 0), 0) / competitors.length;
  if (avgBacklinks > 200) {
    recommendations.push(`Build backlinks - competitors average ${Math.round(avgBacklinks)} backlinks`);
  }
