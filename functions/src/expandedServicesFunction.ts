 ...
 */
export const initializeExpandedServices = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      if (!validation.valid) {
        functions.logger.error("Service validation failed", {
      ...
      const existingServicesSnapshot = await db
        .collection("services")
        .limit(1)
        .get();
      ...
      if (!existingServicesSnapshot.empty && !forceOverwrite) {
        return {
          success: false,
          message:
            "Services already exist. Use forceOverwrite=true to reinitialize.",
      ...
      if (forceOverwrite) {
        functions.logger.info(
          "Force overwrite enabled, clearing existing services",
        );
        const existingServices = await db.collection("services").get();
        const deletePromises = existingServices.docs.map((doc) =>
          doc.ref.delete(),
        );
        await Promise.all(deletePromises);
        functions.logger.info(
          `Deleted ${existingServices.size} existing services`,
        );
      }
      ...
      functions.logger.error("Failed to initialize expanded services", {
        error,
      });
      throw new functions.https.HttpsError(
        "internal",
 ...
 */
export const getServiceStatistics = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
      ...
      servicesSnapshot.docs.forEach((doc) => {
        ...
        if (service.category) {
          stats.byCategory[service.category] =
            (stats.byCategory[service.category] || 0) + 1;
        }
      });
 ...
 */
export const validateServices = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      servicesSnapshot.docs.forEach((doc) => {
        ...
        if (!service.website)
          errors.push(`Service ${serviceId} missing website`);
        if (!service.description)
          errors.push(`Service ${serviceId} missing description`);
        if (!service.category)
          warnings.push(`Service ${serviceId} missing category`);
        ...
        if (
          !service.applicableVehicles ||
          service.applicableVehicles.length === 0
        ) {
 ...
 */
export const listServicesByWebsite = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
      throw new functions.https.HttpsError(
        "unauthenticated",
    ...
    try {
      ...
      functions.logger.error("Failed to list services by website", {
        website,
        error,
      });
      throw new functions.https.HttpsError(
