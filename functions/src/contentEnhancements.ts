...
const SEO_KEYWORDS: Record<string, string[]> = {
  airport: [
    'airport transfer', 'airport limo', 'airport car service', 'airport shuttle',
    'O\'Hare transportation', 'Midway car service', 'private airport transfer',
  ],
  corporate: [
    'corporate transportation', 'executive car service', 'business travel',
    'corporate shuttle', 'meeting transportation', 'executive limo',
  ],
  wedding: [
    'wedding transportation', 'wedding limo', 'bridal car service',
    'wedding guest shuttle', 'luxury wedding car', 'wedding party bus',
  ],
  partyBus: [
    'party bus rental', 'bachelor party bus', 'bachelorette party bus',
    'nightclub transportation', 'birthday party bus', 'event transportation',
  ],
};
...
export const generateLocationFAQ = functions.https.onCall(
  async (data: { locationId: string; serviceId: string; websiteId: string }) => {
    const { locationId, serviceId, websiteId } = data;

    functions.logger.info('Generating location-specific FAQ', { locationId, serviceId });
    ...
    if (!location || !service) {
      throw new functions.https.HttpsError('not-found', 'Location or service not found');
    }

    // Get location-specific questions or defaults
    const baseQuestions = LOCATION_FAQ_TEMPLATES[locationId] || LOCATION_FAQ_TEMPLATES.default;
...
Return optimized content as JSON with same structure plus:
{
  ...
  "seoMetrics": {
    ...
    try {
      ...
      throw new functions.https.HttpsError('internal', 'SEO optimization failed');
    }
  }
);
...
export const generateABVariants = functions.https.onCall(
  async (data: { contentId: string; variantFocus: 'headline' | 'cta' | 'features' | 'full' }) => {
    const { contentId, variantFocus } = data;

    functions.logger.info('Generating A/B variants', { contentId, variantFocus });
    ...
    const focusInstructions: Record<string, string> = {
      headline: 'Create alternative headlines/titles that test different emotional appeals (urgency, exclusivity, value)',
      cta: 'Create alternative call-to-action text that tests different motivations (booking, inquiry, urgency)',
      features: 'Reorganize and rephrase features to test different value propositions',
...
Also include:
{
  ...
  "abTestMetadata": {
    ...
    try {
      ...
      await db.collection('content_ab_tests').doc(testId).set({
        originalContentId: contentId,
        variantFocus,
        variantA: {
      ...
      throw new functions.https.HttpsError('internal', 'A/B variant generation failed');
    }
  }
);
...
export const createContentVersion = functions.https.onCall(
  async (data: { contentId: string; versionNote?: string }) => {
    ...
    const lastVersion = versionsSnapshot.empty ? 0 : versionsSnapshot.docs[0].data().version;
    const newVersion = lastVersion + 1;

    // Store version
    const versionId = `${contentId}_v${newVersion}`;
    await db.collection('content_versions').doc(versionId).set({
      contentId,
      version: newVersion,
...
export const rollbackContentVersion = functions.https.onCall(
  async (data: { contentId: string; targetVersion: number }) => {
    ...
    if (!versionDoc.exists) {
      throw new functions.https.HttpsError('not-found', `Version ${targetVersion} not found`);
    }
