...
export const aiModelRouter = functions.https.onCall(async (data, context) => {
  try {
    ...
    const finalModel = taskModelMap[task.toLowerCase()] || defaultModel;
...
export const generateWeeklyPerformanceEmail = functions.pubsub
...
export const exportAnalyticsToGSHeets = functions.https.onCall(
  async (data, context) => {
...
export const syncWithQuickbooks = functions.https.onCall(
  async (data, context) => {
...
export const fetchLiveGasPrices = functions.pubsub
...
export const triggerCustomWebhook = functions.https.onCall(
  async (data, context) => {
...
export const bulkUpdateVehicleStatus = functions.https.onCall(
  async (data, context) => {
...
export const auditUserPermissions = functions.https.onCall(
  async (data, context) => {
    const users = await admin.auth().listUsers();
    const anomalies = users.users.filter(
      (u) =>
        u.customClaims?.role === "SuperAdmin" &&
        !u.email?.endsWith("@royalcarriagelimo.com"),
    );
    return { anomalies };
  },
);
