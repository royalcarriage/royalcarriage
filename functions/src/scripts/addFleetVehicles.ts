...
export const fleetVehiclesData: FleetVehicle[] = [
  ...
  // ===== EXECUTIVE VANS (2 vehicles) =====
  {
    ...
    applicableLocations: 'all',
    description: 'The Mercedes Sprinter Van is the premier choice for group airport transfers and corporate shuttles. With seating for 14 passengers and generous luggage space, this high-roof van combines Mercedes quality with practical group transportation. Perfect for corporate shuttles, tour groups, wedding guest transfers, and convention transportation. Features comfortable captain chairs, climate control, and power outlets for every passenger.',
    seoKeywords: [
  ...
  // ===== COACH BUSES (1 vehicle) =====
  {
    ...
    applicableLocations: 'all',
    description: 'The Full-Size Motor Coach is the ultimate solution for large group transportation. With seating for 50+ passengers, this coach bus features reclining seats, restroom facility, overhead storage, and large luggage bays. Perfect for corporate tours, conventions, church groups, school trips, and large airport transfers. Wheelchair accessible with PA system and climate control. Provides comfortable, efficient transportation for any large group event or charter.',
    seoKeywords: [
 ...
 */
export function getVehiclesByCategory(category: FleetVehicle['category']): FleetVehicle[] {
 ...
 */
export function getVehiclesByCapacity(minCapacity: number, maxCapacity?: number): FleetVehicle[] {
  if (maxCapacity) {
    return fleetVehiclesData.filter(v => v.capacity >= minCapacity && v.capacity <= maxCapacity);
  }
