 ...
 */
export const generateFAQForCity = functions.https.onCall(async (data, context) => {
    try {
        const { city } = data;
        ...
        if (cached.exists) {
            ...
            if (cacheData && new Date(cacheData.generatedAt.toDate()).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000) {
                functions.logger.info('[generateFAQForCity] Returning cached FAQ', { city });
            ...
            faq: [
                {
                    ...
                    answer: 'You can book through our website, mobile app, or by calling our customer service team.',
                }
            ]
        });
 ...
 * Uses Gemini 1.5 Flash for cost efficiency
 */
export const summarizeCustomerReviews = functions.https.onCall(async (data, context) => {
    try {
 ...
 */
export const translatePageContent = functions.https.onCall(async (data, context) => {
    try {
...
export const detectBrokenInternalLinks = functions.https.onCall(async (data, context) => {
    // Scan all pages for 404 links
    return { broken: [] };
});

export const generateSitemapEntries = functions.https.onCall(async (data, context) => {
 ...
 */
export const suggestSocialCaptions = functions.https.onCall(async (data, context) => {
    try {
        ...
        const platformGuide = platformGuides[platform.toLowerCase()] ||
...
export const analyzeSentimentOfFeedback = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        try {
            ...
            if (!feedbackText) {
                functions.logger.warn('[analyzeSentimentOfFeedback] Empty feedback text');
                return;
            }

            functions.logger.info('[analyzeSentimentOfFeedback] Analyzing sentiment', {
                feedbackId: snap.id,
...
export const optimizeImageMetadata = functions.https.onCall(async (data, context) => {
    const { imageId } = data;
    // Update alt tags and captions with AI
    return { success: true };
});

export const createFleetHighlightVideo = functions.https.onCall(async (data, context) => {
