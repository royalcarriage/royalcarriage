  ...
  if (command === "status") {
    ...
    const checks = await Promise.all([
      db
        ...
        .doc("health")
        .get()
        .then(() => ({ name: "Firestore", status: "online", latency: 35 })),
...
export const executeTerminalCommand = functions.https.onCall(
  async (data: CommandRequest, context): Promise<CommandResult> => {
    ...
    try {
      switch (type) {
        case "firebase":
          output = await executeFirebaseCommand(
            command.replace(/^(firebase|fb)\s+/, ""),
          );
          break;
        case "gemini":
          output = await executeGeminiCommand(
            command.replace(/^(gemini|ai)\s+/, ""),
            args,
          );
          break;
        case "git":
          ...
          break;
        case "system":
          output = await executeSystemCommand(
            command.replace(/^system\s+/, ""),
          );
          break;
        default:
...
export const getSystemMetrics = functions.https.onCall(
  async (data, context): Promise<Record<string, any>> => {
    ...
    const [bookingsSnap, contentSnap, importsSnap, commandsSnap] =
      await Promise.all([
