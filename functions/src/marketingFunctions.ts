...
export const scheduleSocialPost = functions.https.onCall(
  async (data, context) => {
    verifyAuth(context);
...
export const getAudienceInsights = functions.https.onCall(
  async (data, context) => {
    verifyAuth(context);
    // Mock data based on customer aggregate
    return {
      topLocations: ["Chicago", "Naperville"],
...
export const generateSitemap = functions.https.onRequest(async (req, res) => {
  ...
  res.send(
    "<urlset><url><loc>https://royalcarriagelimoseo.web.app/</loc></url></urlset>",
  );
});
