...
export const getTripDetails = functions.https.onCall(async (data, context) => {
    ...
    if (!doc.exists) throw new functions.https.HttpsError('not-found', 'Trip not found');
    return doc.data();
});
...
export const calculateTripFare = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
...
export const validateTripAddress = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
    const { address } = data;
    // Mock validation
...
export const archiveOldTrips = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
...
export const getDispatchStats = functions.https.onCall(async (data, context) => {
    verifyAuth(context);
    const now = new Date();
