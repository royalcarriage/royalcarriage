 ...
 */
export function validateAdsMetric(data: any): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (
    ...
    data.revenue !== null &&
    (isNaN(data.revenue) || data.revenue < 0)
  ) {
  ...
  if (
    data.ctr !== undefined &&
    data.ctr !== null &&
    (isNaN(data.ctr) || data.ctr < 0 || data.ctr > 100)
  ) {
...
export function normalizeAdsMetric(data: any): AdsMetric {
  const normalized: AdsMetric = {
    ...
    targetLocation: data.targetLocation
      ? String(data.targetLocation).trim()
      : undefined,
