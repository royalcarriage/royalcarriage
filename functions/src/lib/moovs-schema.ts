 ...
 */
export function validateMoovsTrip(data: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (data.tip !== undefined && data.tip !== null && (isNaN(data.tip) || data.tip < 0)) {
  ...
  if (data.distance !== undefined && data.distance !== null && (isNaN(data.distance) || data.distance < 0)) {
...
export function normalizeMoovsTrip(data: any): MoovsTrip {
  return {
    ...
    passengerPhone: data.passengerPhone ? String(data.passengerPhone).trim() : undefined,
    passengerEmail: data.passengerEmail ? String(data.passengerEmail).trim() : undefined,
    ...
    paymentMethod: data.paymentMethod ? String(data.paymentMethod).trim() : undefined,
