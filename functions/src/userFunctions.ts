...
export const updateUser = functions.https.onCall(async (data, context) => {
  ...
  if (data.updates.role) {
    await admin
      .auth()
      .setCustomUserClaims(data.uid, { role: data.updates.role });
  }
  return { success: true };
});
...
export const sendSystemNotification = functions.https.onCall(
  async (data, context) => {
    verifyAdmin(context);
...
export const clearSystemCache = functions.https.onCall(
  async (data, context) => {
    verifyAdmin(context);
    // Mock cache clear
    return { success: true };
