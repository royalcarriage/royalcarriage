...
export const createLedgerEntry = functions.https.onRequest(async (req, res) => {
  if (req.method !== "POST") {
    return res.status(405).send("Method Not Allowed");
  }

  try {
    const { amount, date, description, category, accountId, transactionType } =
      req.body;
...
export const onTripComplete = functions.firestore
  ...
  .onCreate(
    async (
      snap: Change<admin.firestore.DocumentSnapshot>,
      ...
      functions.logger.info(
        `Processing trip completion for Trip ID: ${tripId}`,
      );
      ...
      try {
        ...
        const ledgerEntry = {
          ...
          date:
            tripCompletionData.completedAt ||
        ...
        functions.logger.error(
          `Error processing trip completion for ${tripId}:`,
          error,
        );
      }
