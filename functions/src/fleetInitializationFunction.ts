 ...
 */
export const initializeFleetVehicles = functions.https.onCall(
  async (data, context) => {
    ...
    try {
      ...
      const existingVehiclesSnapshot = await db
        ...
        try {
          ...
          if (options.dryRun) {
            functions.logger.info(
              `[DRY RUN] Would create/update vehicle: ${vehicle.id}`,
            );
            results.created++;
          } else {
 ...
 */
export const getAllFleetVehicles = functions.https.onCall(
  async (data, context) => {
    try {
      const db = admin.firestore();
 ...
 */
export const updateFleetVehicle = functions.https.onCall(
  async (data, context) => {
    // Security: Require authentication
    if (!context.auth) {
      throw new functions.https.HttpsError(
 ...
 */
export const deleteFleetVehicle = functions.https.onCall(
  async (data, context) => {
    // Security: Require authentication
    if (!context.auth) {
      throw new functions.https.HttpsError(
